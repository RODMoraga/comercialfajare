...
$file = fopen('../../logs/pdfcontadora_'.date('His').'.sql','w') ;
fwrite($file, $sql) ;
fclose($file) ;
...
/*$logger = "../../logs/log_ordernote_print".(string)date('YmdHis').'.txt';
$file = fopen($logger, 'w');
fwrite($file, 'Fecha y Hora:'.date('d-m-Y H:i:s').' ');
fwrite($file, 'num_line = '.strval($num_line));
fclose($file);*/
...
if (!file_exists($path."/documents/pdf")) {
    mkdir($path."/documents/pdf", 0777, true);
}

$pdf->Output('F',$path.'/documents/pdf/'.$folio.'.pdf');
?>